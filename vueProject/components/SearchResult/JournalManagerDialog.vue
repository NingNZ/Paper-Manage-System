      ...
      <li v-for="(item, index) in journals" :key="index" class="item-row">
        ...
        <template v-else>
          ...
          <div>
            ...
            &nbsp;
            <img src="../../assets/delete.svg" class="icon-action" @click="deleteJournal(index)" />
          </div>
        </template>
      </li>
    </ul>
...
const addJournal = () => {
  journals.value.push(`新期刊${journals.value.length + 1}`)
}
...
const saveEdit = (index) => {
  if (editText.value.trim()) {
    journals.value[index] = editText.value.trim()
  }
  cancelEdit()
}
